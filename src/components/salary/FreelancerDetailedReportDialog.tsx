...
import { generateFreelancerDetailedReportPDF } from '@/components/salary/FreelancerDetailedReportPDF';
...
interface FreelancerDetailedReportDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  freelancer: FreelancerData | null;
}

const FreelancerDetailedReportDialog = ({ 
  open, 
  onOpenChange, 
  freelancer 
}: FreelancerDetailedReportDialogProps) => {
      ...
      <DialogContent className="w-[95vw] max-w-[500px] md:max-w-[600px] max-h-[70vh] md:max-h-[90vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2 text-base">
            ...
            <div className="text-left min-w-0 flex-1">
              <h2 className="text-base font-bold truncate">{freelancer.full_name}</h2>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 min-h-0">
          ...
          <div className="grid grid-cols-2 gap-3 p-3 bg-muted/30 rounded-lg flex-shrink-0">
            <div className="text-center">
              <div className="text-lg font-bold text-primary">₹{freelancer.total_earnings.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Total Earnings</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-green-600">₹{freelancer.paid_amount.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Paid Amount</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-orange-600">₹{freelancer.pending_amount.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Pending Amount</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-blue-600">{freelancer.total_assignments}</div>
          ...
          <div className="space-y-3 p-3 border rounded-lg">
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-xs">
                <div className="flex items-center gap-1">
                  ...
                  <span className={`text-xs font-medium ${getRoleTextColor(freelancer.role)}`}>
                    {displayRole(freelancer.role)}
                  </span>
                </div>
                {freelancer.phone && (
                  <p><span className="font-medium">Phone:</span> <span className="break-all">{freelancer.phone}</span></p>
                )}
                {freelancer.email && (
                  <p><span className="font-medium">Email:</span> <span className="break-all">{freelancer.email}</span></p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-xs">
                <p><span className="font-medium">Assignments:</span> ₹{freelancer.assignment_earnings.toLocaleString()}</p>
                <p><span className="font-medium">Tasks:</span> ₹{freelancer.task_earnings.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-xs">
                <p><span className="font-medium">Total Tasks:</span> {freelancer.total_tasks}</p>
                <p><span className="font-medium">Completed:</span> {freelancer.completed_tasks}</p>
                <p><span className="font-medium">Rate:</span> {freelancer.total_tasks > 0 ? Math.round((freelancer.completed_tasks / freelancer.total_tasks) * 100) : 0}%</p>
              </div>
            </div>
          </div>
          ...
          {freelancer.detailedEventBreakdown && freelancer.detailedEventBreakdown.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {freelancer.detailedEventBreakdown.map((event, index) => (
          ...
          <div className="space-y-3 p-3 bg-muted/30 rounded-lg">
            <div className="text-xs text-muted-foreground text-center">
              Events: {freelancer.detailedEventBreakdown?.length || 0} | Payments: {freelancer.paymentHistory?.length || 0}
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-3 space-y-3 flex-shrink-0">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Button 
              onClick={async () => {
                await generateFreelancerDetailedReportPDF(freelancer);
              }}
              className="h-10 text-sm"
            >
...
export default FreelancerDetailedReportDialog;