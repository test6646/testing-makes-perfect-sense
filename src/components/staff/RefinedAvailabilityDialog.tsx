...
const eventNotificationFormSchema = z.object({
  ...
  globalMessage: z.string().min(1, 'Please enter a message'),
});
  ...
  const onEventNotificationSubmit = async (values: z.infer<typeof eventNotificationFormSchema>) => {
    ...
    try {
      ...
      for (const assignment of assignmentsToNotify) {
        ...
        if (assignment.phone && messageToSend) {
      ...
      toast({
        ...
        description: `Successfully sent notifications to ${assignmentsToNotify.length} staff members.`,
      });
                ...
                <Form {...eventForm}>
                  <form onSubmit={eventForm.handleSubmit(onEventNotificationSubmit)} className="space-y-6">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Global Message *</FormLabel>
                          <FormControl>
                            <Textarea
                              {...field}
