...
export const useTasks = () => {
  ...
  const deleteTask = async (taskId: string) => {
    try {
      ...
      // Send deletion notifications in background
      if (taskData && currentFirmId) {
        supabase.functions.invoke('send-task-deletion-notifications', {
          body: {
            taskData,
            firmId: currentFirmId
          }
        }).catch(error => {
          console.warn('Failed to send task deletion notifications:', error);
        });

        // Sync deletion to Google Sheets
        import('@/services/googleSheetsSync').then(({ syncTaskInBackground }) => {
          syncTaskInBackground(taskId, currentFirmId, 'delete');
        }).catch(() => {});
      }
