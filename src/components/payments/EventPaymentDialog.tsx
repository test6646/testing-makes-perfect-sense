...
const EventPaymentDialog = ({ event, open, onOpenChange, onPaymentClick }: EventPaymentDialogProps) => {
  ...
  const getEventTypeColor = (eventType: string) => {
    const colors = {
      ...
      'Pre-Wedding': 'bg-pink-100 text-pink-800 border-pink-200',
      'Ring-Ceremony': 'bg-purple-100 text-purple-800 border-purple-200',
      'Maternity Photography': 'bg-green-100 text-green-800 border-green-200',
  ...
  const hasPendingAmount = (event.balance_amount || 0) > 0;
          ...
          <div className="bg-muted/30 rounded-xl p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${event.photo_editing_status ? 'bg-green-500' : 'bg-gray-300'}`} />
                ...
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${event.video_editing_status ? 'bg-green-500' : 'bg-gray-300'}`} />
              ...
              <div className="space-y-2">
                ...
                {hasPendingAmount && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-orange-600">₹{event.balance_amount?.toLocaleString() || '0'}</span>
                  </div>
                )}
              </div>
