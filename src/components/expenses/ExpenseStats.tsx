...
import { MoneyBag02Icon, DollarCircleIcon, Calendar01Icon, ChartBarLineIcon } from 'hugeicons-react';
import { useGlobalExpenseStats } from '@/hooks/useGlobalExpenseStats';
...
const ExpenseStats = () => {
  ...
  const calculateStats = () => {
    ...
    const thisMonthExpenses = expenses
      .filter(expense => format(new Date(expense.expense_date), 'yyyy-MM') === currentMonth)
      .reduce((sum, expense) => sum + (Number(expense.amount) || 0), 0);
    const thisYearExpenses = expenses
      .filter(expense => format(new Date(expense.expense_date), 'yyyy') === currentYear)
      .reduce((sum, expense) => sum + (Number(expense.amount) || 0), 0);
  ...
  return (
    <div className="flex gap-1 sm:gap-3 md:gap-4 w-full">
      ...
      <StatCard
        title="This Year"
        value={`₹${stats.thisYearExpenses.toLocaleString()}`}
