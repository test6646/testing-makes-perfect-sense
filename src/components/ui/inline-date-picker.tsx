...
export function InlineDatePicker({
  ...
  const handleDateChange = (day?: string, month?: string, year?: string) => {
    ...
    if (finalDay && finalMonth && finalYear) {
      // Use UTC to avoid timezone issues - month is 0-indexed in Date constructor
      const newDate = new Date(Date.UTC(parseInt(finalYear), parseInt(finalMonth), parseInt(finalDay)))
      onSelect?.(newDate)
    }
  }
