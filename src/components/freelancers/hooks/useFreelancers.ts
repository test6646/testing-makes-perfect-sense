...
export const useFreelancers = (onRefetch?: () => void) => {
  ...
  const createFreelancer = async (data: FreelancerFormData) => {
    ...
    try {
      ...
      // Trigger refresh in any open event form dialogs
      if ((window as any).refreshEventFormFreelancers) {
        (window as any).refreshEventFormFreelancers();
      }

      // Trigger external refetch to update FilteredManagementCore data
      if (onRefetch) {
        onRefetch();
      }
