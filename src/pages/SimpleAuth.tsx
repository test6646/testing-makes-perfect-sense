...
import { Loading02Icon, AlertCircleIcon, CheckmarkCircle01Icon, Building06Icon, EyeIcon, ViewOffIcon, Mail01Icon, LockIcon, UserIcon, AiPhone01Icon, TelegramIcon } from 'hugeicons-react';
...
const SimpleAuth = () => {
  ...
  // Only redirect authenticated users with confirmed emails
  useEffect(() => {
        ...
        options: {
          emailRedirectTo: `${window.location.origin}/`,
            data: {
              ...
              firm_id: signUpData.role !== 'Admin' ? signUpData.firmId : null,
              telegram_chat_id: signUpData.telegramChatId
            }
        }
      });
            ...
            <TabsContent value="signin" className="space-y-3 mt-4">
             ...
             <TabsContent value="signup" className="space-y-3 mt-4">
               <form onSubmit={handleSignUp} className="space-y-3">
                     ...
                     <div className="space-y-2">
                       <Label htmlFor="signup-telegram" className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          value={signUpData.telegramChatId}
                          onChange={(e) => setSignUpData({ ...signUpData, telegramChatId: e.target.value })}
                          className="pl-4 h-10"
                        />
                     </div>
                   </div>
                 <div className={`grid gap-3 ${signUpData.role !== 'Admin' && signUpData.role ? 'grid-cols-2' : 'grid-cols-1'}`}>
                   <div className="space-y-2">
                   ...
                   {signUpData.role !== 'Admin' && signUpData.role && (
                     <div className="space-y-2">
                       ...
                       <Select value={signUpData.firmId} onValueChange={(value) => setSignUpData({ ...signUpData, firmId: value })}>
                         <SelectTrigger className="h-10">
                           <SelectValue placeholder={loadingFirms ? "Loading firms..." : "Select a firm to join"} />
                         </SelectTrigger>
                         <SelectContent>
                           {firms.map((firm) => (
                 ...
                 {signUpData.role !== 'Admin' && signUpData.role && firms.length === 0 && !loadingFirms && (
                   <Alert>
