...
export class CentralizedErrorHandler {
  ...
  private categorizeError(error: any): Omit<ErrorResponse, 'message'> {
    ...
    if (errorMessage.includes('whatsapp') || errorMessage.includes('session')) {
      return {
        code: 'WHATSAPP_ERROR',
        severity: 'medium',
        shouldRetry: true,
        userMessage: 'WhatsApp connection issue. Please check your connection status.'
      };
    }
...
export const handleWhatsAppError = (error: any, action: string) => {
  return CentralizedErrorHandler.getInstance().handleError(error, {
    component: 'WhatsApp',
    operation: action
  });
