...
export const useExpenseExportConfig = (): ExportConfig => {
  return useMemo(() => ({
    ...
    exportFunction: async (data, filterType, filterValue) => {
      await generateExpenseReportPDF(data, filterType, filterValue);
    },
    getPreviewData: (data) => ({
      count: data.length,
      summary: {
  ...
  return useMemo(() => ({
    ...
    filterTypes: [
      ...
      { 
        value: 'status', 
        label: 'By Status',
        options: ['Pending', 'In Progress', 'Completed', 'On Hold', 'Waiting for Response', 'Accepted', 'Declined']
          .map(status => ({ value: status, label: status }))
      }
    ],
    exportFunction: async (data, filterType, filterValue) => {
      await generateTaskReportPDF(data, filterType, filterValue);
    },
    getPreviewData: (data) => ({
      count: data.length
    })
...
export const useSalaryExportConfig = (staffData: any[], freelancerData: any[], totalStats: any): ExportConfig => {
  return useMemo(() => ({
    ...
    exportFunction: async (data, filterType, filterValue) => {
      await generateSalaryReportPDF(staffData, freelancerData, filterType as 'staff' | 'freelancers' | 'all', totalStats);
    },
    getPreviewData: (data, filterType, selectedValue) => {
      ...
      switch (filterType) {
        case 'staff':
          ...
          break;
        case 'freelancers':
          if (selectedValue === 'all_freelancers') {
            summaryData = {
