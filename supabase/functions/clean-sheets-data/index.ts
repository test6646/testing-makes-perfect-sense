...
// Helper function to clear all data except headers
async function clearSheetData(accessToken: string, spreadsheetId: string, sheetName: string) {
  console.log(`🧹 Clearing data from ${sheetName} sheet...`);
  ...
  if (rowCount <= 1) {
    console.log(`✅ ${sheetName} sheet only has headers, nothing to clear`);
    return;
  }
  ...
  console.log(`✅ Cleared all data from ${sheetName} sheet (kept headers)`);
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!firmId) {
      console.error('❌ Missing firmId parameter');
    ...
    const sheets = ['Master Events', 'Clients', 'Tasks', 'Expenses'];
    const clearedSheets = [];
    ...
    for (const sheetName of sheets) {
      try {
        ...
        console.warn(`⚠️ Could not clear ${sheetName}: ${error.message}`);
      }
    }
