...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (isDuplicate) {
      console.log(`Staff already exists: ${profile.full_name} (${userId})`);
      return new Response(JSON.stringify({
        success: true,
    ...
    if (!formatRes.ok) {
      ...
      console.warn('Failed to apply Lexend formatting:', formatError);
    } else {
      console.log('✅ Staff added to sheet with Lexend font');
    }
    return new Response(JSON.stringify({
      success: true,
