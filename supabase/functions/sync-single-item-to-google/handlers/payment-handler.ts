...
export async function syncStaffPaymentToSheet(
  ...
  console.log(`✅ Synced staff payment ₹${staffPayment.amount} to ${staffPayment.staff?.full_name} to Google Sheets`);
  return staffPayment;
}
...
export async function syncFreelancerPaymentToSheet(
  ...
  await updateOrAppendToSheet(accessToken, spreadsheetId, 'Expenses', expenseData, 0);
  console.log(`✅ Synced freelancer payment ₹${freelancerPayment.amount} to ${freelancerPayment.freelancer?.full_name} to Google Sheets`);
