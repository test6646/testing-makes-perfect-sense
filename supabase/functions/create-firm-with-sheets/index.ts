...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.log('📱 PHASE 6: Initializing WhatsApp session...');
      try {
        ...
        if (waSessionError) {
          console.warn('⚠️ WhatsApp session creation failed:', waSessionError.message);
        } else {
          console.log('✅ WhatsApp session initialized with firm data');
        }
      } catch (waError) {
        console.warn('⚠️ WhatsApp session setup error:', waError);
      }
